 ...
 * 
 */
public class ProviceServerEncoder extends MessageToByteEncoder<Package> {
	
	private static Logger logger = LoggerFactory.getLogger(ProviceServerEncoder.class);
...
//			logger.info("服务端-->客户端<{}>的报文是：{}",ctx.channel().remoteAddress().toString(),pkg.getClient_sync_info() + pkg.getZip_flag() + pkg.getEncrypt_flag() + pkg.getVersion()
//			+ pkg.getMsg_type() +"content" + pkg.getMac());
//		}else {
//			logger.info("服务端-->客户端<{}>的报文是：{}",ctx.channel().remoteAddress().toString(),BytesUtil.bytesToHexString(result));
//		}
