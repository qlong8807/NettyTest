 ...
 * 
 */
public class ProviceClientDecoder extends ByteToMessageDecoder {
	
	private static Logger logger = LoggerFactory.getLogger(ProviceClientDecoder.class);
	/**
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in,
		...
		try {
			...
			logger.error("客户端<--服务端的包长度<{}>不是数字，直接返回系统错误。",head_len);
			com.cp.client.Package pkg = new Package();
		...
		if("1".equals(pkg.getEncrypt_flag())) {
			...
			if(Constant.PRO_DOWN_REQUEST_CMD.equals(pkg.getMsg_type()) || Constant.PRO_UP_REQUEST_CMD.equals(pkg.getMsg_type())
					|| Constant.PRO_REQUEST_DESKEY.equals(pkg.getMsg_type()) || Constant.PRO_RESPONSE_DESKEY.equals(pkg.getMsg_type())) {
				//不做任何操作
