...
public class EchoDelimiterBaseClient {
	private final String host;
	private final int port;

	public EchoDelimiterBaseClient(String host, int port) {
		this.host = host;
		this.port = port;
	}

	public static void main(String[] args) throws Exception {
		new EchoDelimiterBaseClient("127.0.0.1", 8080).start();
		// new EchoClient("192.168.0.55", 10027).start();
	}
		...
		try {
			...
			b.handler(new ChannelInitializer<SocketChannel>() {
				public void initChannel(SocketChannel ch) throws Exception {
					ch.pipeline().addLast(
							new DelimiterBasedFrameDecoder(1024 * 1024,Unpooled
	...
	private class EchoClientHandler extends ChannelInboundHandlerAdapter {
		...
		public void channelActive(ChannelHandlerContext ctx) {
			String s = "hello Netty end";
			ctx.writeAndFlush(s);// 加了StringEncoder编码器就可以直接发送String，要不还得转换为ByteBuffer
			// 拆包问题
			/*
