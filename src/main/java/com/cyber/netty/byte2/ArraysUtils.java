...
public final class ArraysUtils {
	 ...
	 * @return
	 */
	public final static byte[] arrayappend(final byte[] dest,
			int startIndex, final byte[] src) {
		if (src.length + startIndex > dest.length)
			throw new IndexOutOfBoundsException("length = " + dest.length
					+ " , total length = " + (startIndex + src.length));
